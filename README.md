...
A Python library for operating an LCD1602 (16 x 2 character display) device over an I2C interface. This library has been confirmed to work on Raspberry Pi 3B and 5.
...
5 x 8 dot custom font may be registered in CGRAM using character codes in the range 0x00 to 0x07.
...
This library requires the `smbus` package for I2C communication. It is usually installed as standard on Raspberry Pi OS, but if it is not, install it with the following command.
...
Alternatively, run the demo script to see various features in action. Before running it, change the I2C address in the script to the address you confirmed above step.
...
#### **`write_text(text)`**
Write text starting at the cursor position and move the cursor to the end of the written text. If the cursor is outside the display, the text is written starting at the beginning of the current row.
* **`text`** (str): String to display.

#### **`write_bytes(btext)`**
Write text in bytes starting at the cursor position and move the cursor to the end of the written text. If the cursor is outside the display, the text is written starting at the beginning of the current row.
* **`btext`** (bytes): Bytes to display.

#### **`write_text_at(text, row, col=0)`**
Write text starting at the specified display position and move the cursor to the end of the written text.
* **`text`** (str): String to display.
...
Write text in bytes starting at the specified display position and move the cursor to the end of the written text.
* **`btext`** (bytes): Bytes to display.
* **`row`** (int): Row index (0 or 1).
* **`col`** (int): Column index (Valid range: 0-15).
* **Raises**: `ValueError` if `row` or `col` is out of the range.
...
Write text on the specified row with left, center, or right alignment on the display. The cursor is moved to the end of the written text.
* **`text`** (str): String to display.
* **`row`** (int): Row index (0 or 1).
...
Write text in bytes on the specified row with left, center, or right alignment on the display. The cursor is moved to the end of the written text.
* **`btext`** (bytes): Bytes to display.
* **`row`** (int): Row index (0 or 1).
* **`alignment`** (LCDAlignment): `LEFT`, `CENTER`, or `RIGHT`.
* **`fill_space`** (bool): `True` to clear any parts other than the text with space characters, `False` to leave them unchanged.
...
Write text starting at the cursor position and move the cursor to the end of the written text.
* **`text`** (str): String to display.

#### **`write_bytes_buffer(btext)`**
Write text in bytes starting at the cursor position and move the cursor to the end of the written text.
* **`btext`** (bytes): Bytes to display.

#### **`write_text_buffer_at(text, row, col=0)`**
Write text starting at the specified buffer position and move the cursor to the end of the written text.
* **`text`** (str): String to display.
* **`row`** (int): Row index (0 or 1).
...
Write text in bytes starting at the specified buffer position and move the cursor to the end of the written text.
* **`btext`** (bytes): Bytes to display.
* **`row`** (int): Row index (0 or 1).
* **`col`** (int): Column index (Valid range: 0-39).
* **Raises**: `ValueError` if `row` or `col` is out of the range.
...
#### **`clear_all()`**
Clear the entire buffer by filling it with space characters, reset the buffer position to its initial state, and set the cursor to the top-left corner of the display.

#### **`clear_row(row)`**
Clear the display of the specified row by filling it with space characters, and set the cursor to the beginning of that row.
* **`row`** (int): Row index (0 or 1).
* **Raises**: `ValueError` if `row` is out of the range.

#### **`clear(row, col=0, length=0)`**
Clear a specified length in the display by overwriting it with space characters, starting at the specified display position. Then move the cursor to the end of the written space characters.
* **`row`** (int): Row index (0 or 1).
* **`col`** (int): Column index (Valid range: 0-15).
...
Clear the buffer of the specified row by filling it with space characters, reset the buffer position to its initial state, and set the cursor to the beginning of that row.
* **`row`** (int): Row index (0 or 1).
* **Raises**: `ValueError` if `row` is out of the range.

#### **`clear_buffer(row, col=0, length=0)`**
Clear a specified length in the buffer by overwriting it with space characters, starting at the specified buffer position. Then move the cursor to the end of the written space characters.
* **`row`** (int): Row index (0 or 1).
* **`col`** (int): Column index (Valid range: 0-39).
...
#### **`set_home()`**
Reset the buffer position to its initial state, and set the cursor to the top-left corner of the display.
...
Move the cursor by the specified amount. If the cursor goes off the display, it will stay at the left or right edge of the display.
...
Reset the buffer position to its initial state, and set the cursor to the beginning of the specified row.
* **`row`** (int): Row index (0 or 1).
* **Raises**: `ValueError` if `row` is out of the range.
...
#### **`turn_off()`**
Turn off the backlight, disable the display and cursor, and clear the buffer.
